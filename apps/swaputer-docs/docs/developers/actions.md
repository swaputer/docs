...
The `router` and `authorizedExecutor` fields must describe the path that will actually submit the transaction:

- For a direct DEPLOY or CALL through the official Uniswap Universal Router, bind `router` to that Universal Router, set `authorizedExecutor` to the zero address, use its `V4_SWAP` command, and encode the complete signed envelope into the v4 swap's `hookData`.
- For a market, sETH vault, or another executor-bound application, bind `router` to the Swaputer Router and `authorizedExecutor` to the application contract required by that flow.

Both router addresses and their runtime code hashes must come from the same verified release manifest as the World. A client must never sign for one route and submit through the other.
...
- **Authorized executor:** the address the actor permits to submit the transaction, such as a vault or market.
